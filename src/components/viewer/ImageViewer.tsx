...
export function ImageViewer({ url, size }: ImageViewerProps) {
  ...
  // Center image after it loads (Fix 1: delegate to centerImage)
  useEffect(() => {
    if (!dimensions || !containerRef.current || initializedRef.current) return;
    initializedRef.current = true;
    centerImage();
  }, [dimensions, centerImage]);
  ...
  // Zoom centered on a point (cursor or container center)
  const zoomAt = useCallback(
    (centerX: number, centerY: number, newScale: number) => {
      setScale((prevScale) => {
  ...
  // Wheel zoom — delegates to zoomAt (Fix 2: no duplicated math)
  ...
  // Pan — mouse down on container, move/up on window
  // Fix 4: read from translateRef instead of translate state
  ...
  // Fix 3: single button zoom handler
