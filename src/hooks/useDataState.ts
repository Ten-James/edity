...
export function useDataState(projectId: string) {
  ...
  const saveConnection = async (connection: ConnectionConfig) => {
  ...
  const scanRedisKeys = async (pattern: string = "*", reset: boolean = true) => {
  ...
  const loadSqliteColumns = async (table: string) => {
    const connId = getConnId();
    if (!connId) return;
    try {
  ...
  const executeSqliteQuery = async (sql: string, params?: unknown[]) => {
    const connId = getConnId();
    if (!connId) return;
